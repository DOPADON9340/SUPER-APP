...
const category = ()=> {
    ...
    const [lengthError , setLengthError] = useStates(false);
...
const Block =({ dta, idx, setCategories, categories}) => {
    const[selected, setSelected] = useState();
    const handleClick =(e) =>{
        if (categories.include(data.id)){
    ...
    return(
        ...
        >
            <p styles={{marginBotton: "4px" , fontsize:"18px"}}> {data.id}</p>
            {data.image}
        </div>
    );
};
